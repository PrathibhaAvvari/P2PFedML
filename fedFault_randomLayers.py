...
log_filename = f"min40_crash_test_{TIMEOUT}_randomLayers_log_{NUM_CLIENTS}_{NUM_MACHINES}_{len(faults)}.txt"
...
def client_logic(id, local_ip, ips, faults):
    ...
    while current_round < R_PRIME:
        ...
        # --- Random layer assignment + pull-based layer collection (bandwidth-efficient) ---
        # Only include alive (non-crashed) neighbors in layer assignment
        ...
        # Build per-peer request lists (param names) based on logical-layer assignment
        ...
        # Construct stacked state from pulled layers; fallback to local if missing.
        stacked_state = dict(local_state_np)
        for layer_key, chosen_peer in assignment.items():
            if chosen_peer == id:
                continue
...
def main():
    ...
    print("Starting Federated Learning (Random Layer Stacking)")
    ...
    if local_client_ids:
        ...
        for cid in local_client_ids:
            ...
            print(
                f"Client {cid}: train={train_s:.2f}, comm_io={comm_io_s:.2f}, comm_phase={comm_phase_s:.2f}, comm_total={comm_io_s + comm_phase_s:.2f}, total={total_s:.2f}"
            )
