...
TIMEOUT = int(os.environ.get("FED_TIMEOUT", "25"))        # seconds to wait for layer pulls per round
...
R_PRIME = 100          # maximum rounds
MINIMUM_ROUNDS = 40    # min rounds before termination check
COUNT_THRESHOLD = 5    # consecutive stable rounds needed
...
def _recv_exact(conn, nbytes: int):
    """Reliably read exactly nbytes from conn (handles partial TCP reads)."""
    data = b''
    ...
    try:
        with open("inputf.txt", "r") as file:
            ...
            for i in range(num_faults):
                id_, fr, y = map(int, lines[4 + i].split(','))
                faults.append((id_, fr, y))
...
# --- Option 3: VGG-11 with BN ---
def VGG11BN():
    cfg = [64, 'M', 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512, 'M']
    return VGG(_make_vgg_layers(cfg))


# --- Option 4: VGG-13 with BN ---
def VGG13BN():
    cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512, 'M']
    return VGG(_make_vgg_layers(cfg))


# --- Option 5: VGG-16 with BN ---
def VGG16BN():
    cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M']
    return VGG(_make_vgg_layers(cfg))


# --- Option 6: ResNet-20 (CIFAR-specific, He et al. 2015) ---
...
class _CifarBasicBlock(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != planes:
            # Option A: zero-pad shortcut (no extra params)
            self.shortcut = nn.Sequential(
...
class _CifarResNet(nn.Module):
    """CIFAR ResNet as defined in He et al. 2015.
    depth = 6*n + 2  (n=3 → ResNet-20, n=5 → ResNet-32, n=9 → ResNet-56)
    """
...
def tcp_client(id, target_id, target_ip, message):
    """Send a fire-and-forget message (terminate / broadcast weights) with retries."""
    global retries_list
    retries = TCP_RETRIES
    while retries > 0:
...
def tcp_client_request_layers(requester_id, target_id, target_ip, param_names, current_round, deadline_ts=None):
    """Pull specific named parameters from a peer (layer-request protocol)."""
    global retries_list
    if not param_names:
        return None
...
def tcp_server(id, terminate_flags, local_ip, latest_models, stop_event):
    ...
    Multithreaded TCP server:
      - Handles 'layer_request' messages by responding with the requested parameters.
      - Handles 'terminate' messages by setting stop_event and appending to terminate_flags.
    """
...
def _random_assign_layers_to_participants(local_id, participant_ids, layer_groups):
    """Assign every logical layer to exactly one participant (may be self)."""
    assignment = {}
...
def _random_stack_and_average(local_id, local_state_np, peer_state_by_id, layer_groups):
    """Randomly assign each logical layer to one participant, then average with local."""
...
def client_logic(id, local_ip, ips, faults, model_choice, timing_store):
    ...
    while current_round < R_PRIME:
        ...
        # Make current parameters available for peers to pull.
        with latest_models_lock:
        ...
        alive_neighbors = [n for n in adj[id] if not crash_away_list[n]]
        participants = [id] + alive_neighbors
        assignment = _random_assign_layers_to_participants(id, participants, layer_groups)

        # Group param names per peer to request
        ...
        if current_round >= MINIMUM_ROUNDS:
            ...
            no_recent_crashes = all(
                r not in crashed_in_rounds
...
def main():
    parser = argparse.ArgumentParser(
        description="Federated Learning - Random Layer Stacking with selectable model"
    )
    parser.add_argument(
        "--model",
        type=int,
    ...
    log_filename = (
        f"min40_crash_test_{TIMEOUT}_randomLayers_{model_name.lower().replace('-', '_')}"
        f"_log_{NUM_CLIENTS}_{NUM_MACHINES}_{len(faults)}.txt"
    )
