...
def get_model(model_name):
    ...
    if model_name == 'SimpleCNN':
        ...
        raise ValueError(
            ...
            "SimpleCNN, SimpleCNN10, VGG11BN, VGG13BN, VGG16BN"
        )
...
def broadcast_selected_layers(id, selected_layer_params, current_round, ips):
...
def client_logic(id, local_ip, ips, faults, timing_store):
    ...
    while current_round < R_PRIME:
        ...
        broadcast_selected_layers(id, selected_layer_params, current_round, ips)
        ...
        for param_name, received_values in param_accumulator.items():
            if param_name in new_state_np and len(received_values) > 0:
                ...
                new_state_np[param_name] = np.mean(all_values, axis=0)
        ...
        print(
            f"Client {id} - Round {current_round}: Accuracy: {accuracy:.2f}%, "
            f"Selected {len(selected_layers)} layers, Received from {len(latest_by_sender)} unique peers"
        )
    ...
    print(
        f"Client {id} timing (s): train={train_s:.2f}, comm_io={comm_io_s:.2f} "
        f"[send {send_s:.2f}, recv {recv_s:.2f}], comm_phase={comm_phase_s:.2f}, "
        f"comm_total={comm_io_s + comm_phase_s:.2f}, total={total_s:.2f}"
    )
    print(
        f"Client {id} bytes (app-level): sent={bytes_sent}, recv={bytes_recv}, "
...
def main():
    ...
    _set_start_method()
    manager = mp.Manager()
    model_messages = manager.list([0] * NUM_CLIENTS)
    terminate_messages = manager.list([0] * NUM_CLIENTS)

    start_time = time.time()
    l_share_pct = (100.0 * LAYER_SELECTION_L / TOTAL_LOGICAL_LAYERS) if TOTAL_LOGICAL_LAYERS else 0.0
    print(
        f"Starting Federated Learning (Gradient+Frequency Layer PUSH, Async, Process, "
    ...
    if local_client_ids:
        ...
        for cid in local_client_ids:
            ...
            print(
                ...
                f"total={total_s:.2f}, bytes_sent={bytes_sent}, bytes_recv={bytes_recv}"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Gradient+Frequency layer sharing (async push-based federated learning)"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Number of logical layers to share per round (overrides --l-percent if both are set). "
            "If omitted, defaults to all logical layers."
        ),
    )
    parser.add_argument(
        "--l-percent",
