...
log_filename = f"min40_crash_test_{TIMEOUT}_randomLayers_asyncA_log_{NUM_CLIENTS}_{NUM_MACHINES}_{len(faults)}.txt"
...
def client_logic(id, local_ip, ips, faults):
    ...
    while current_round < R_PRIME:
        ...
        # Async-A: do not wait for long collection windows; pull best-effort within a small per-round budget.
        deadline_ts = time.time() + ASYNC_COMM_BUDGET_S
        ...
        for peer_id, param_names in params_needed_by_peer.items():
            if time.time() >= deadline_ts:
                break
            req_deadline_ts = min(deadline_ts, time.time() + REQUEST_TIMEOUT_S)
...
def main():
    ...
    print("Starting Federated Learning (Random Layer Pulling, Async-A)")
