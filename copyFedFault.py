...
# We keep this minimal and thread-safe, and we do not change the core FL logic.
...
def tcp_client(id, target_id, target_ip, message):
    ...
    while retries > 0:
        try:
            ...
            client.close()
            _add_timing(id, "send_s", time.perf_counter() - _t0)
            break
...
def tcp_server(id, received_weights, terminate_flags, local_ip, latest_models, crash_away_list, prev_list):
    ...
    while True:
        ...
        _add_timing(id, "recv_s", time.perf_counter() - _t0)
...
def client_logic(id, local_ip, ips, faults):
    ...
    # For training time and communication time
    ...
    while current_round < R_PRIME:
        ...
        for epoch in range(EPOCHS_PER_ROUND):
            for data, target in train_loader:
                ...
                optimizer.step()
        train_end = time.time()
        _train_delta = (train_end - train_start)
        total_training_time += _train_delta
        _add_timing(id, "training_s", _train_delta)
        ...
        while (time.time() - t_start) < TIMEOUT:
            time.sleep(0.01)
        comm_end = time.time()
        _wait_delta = (comm_end - comm_start)
        total_comm_time += _wait_delta
        total_wait_time += _wait_delta
        _add_timing(id, "wait_s", _wait_delta)
    ...
    # Keep the original behavior (client prints a summary) but include the new breakdown.
    with _timing_lock:
        _t = dict(client_timing.get(id, {}))
    _send_s = float(_t.get("send_s", 0.0))
    _recv_s = float(_t.get("recv_s", 0.0))
    _wait_s = float(_t.get("wait_s", 0.0))
    _comm_io = _send_s + _recv_s
    _comm_with_wait = _comm_io + _wait_s
    print(
        f"Client {id} finished. "
        f"Training: {total_training_time:.2f}s, "
        f"Comm(IO): {(_comm_io):.2f}s [send {(_send_s):.2f}s, recv {(_recv_s):.2f}s], "
        f"Wait: {total_wait_time:.2f}s, "
        f"Comm(with wait): {(_comm_with_wait):.2f}s, "
        f"Total (train + comm(with wait)): {(total_training_time + _comm_with_wait):.2f}s"
    )
...
def main():
    ...
    # Print per-client timing for clients running on this machine.
    # This is the requested "at last" summary and doesn't alter the algorithm.
    with _timing_lock:
        local_client_ids = sorted([i for i in range(NUM_CLIENTS) if ips[i] == str(CURRENT_MACHINE_IP)])
        timing_snapshot = {cid: dict(client_timing.get(cid, {})) for cid in local_client_ids}

    if local_client_ids:
        print("\nPer-client timing summary (seconds)")
        for cid in local_client_ids:
            t = timing_snapshot.get(cid, {})
            ...
            comm_io = send_s + recv_s
            comm_with_wait = comm_io + wait_s
            total_s = train_s + comm_with_wait
            print(
                f"Client {cid}: "
                f"train={train_s:.2f}, "
                f"send={send_s:.2f}, recv={recv_s:.2f}, "
                f"comm_io={comm_io:.2f}, "
                f"wait={wait_s:.2f}, comm_with_wait={comm_with_wait:.2f}, "
                f"total={total_s:.2f}"
            )
